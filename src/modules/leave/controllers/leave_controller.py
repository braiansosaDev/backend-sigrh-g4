...
@leave_router.get("/{sector_id}", response_model=list[LeavePublic])
...
@leave_router.get("/", response_model=list[LeavePublic])
...
@leave_router.get("/types", response_model=list[LeaveTypePublic])
async def get_leave_types(session: DatabaseSession):
    return leave_service.get_leave_types(session)


@leave_router.get("/types/{leave_type_id}", response_model=LeaveTypePublic)
...
@leave_router.patch("/{leave_id}", response_model=LeavePublic)
